...
**DinkyDash (free, open source)** — our project, and the reason this site exists. Any screen with a browser becomes a family calendar with automatic chore rotation, birthday countdowns, and an AI-written daily brief — a fresh greeting and fun fact every morning, written around your own family, which no hardware calendar offers. It reads [Google](/google-calendar-ical-link/), [Apple iCloud](/icloud-calendar-link/), [Outlook](/outlook-calendar-ics-link/) and [Cozi](/cozi-calendar-display/) calendars (paste each one's sharing link; there's no sign-in) and merges them into a single agenda. Setup takes an afternoon and a bit of terminal comfort ([guide](/getting-started/)); a zero-setup hosted version is [in the works](https://fffwryhvses.typeform.com/to/yxMMhmFs).
